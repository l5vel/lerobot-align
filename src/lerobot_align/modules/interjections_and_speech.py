...
The ``plan`` module emits its deterministic remaining-subtasks state at the
same boundary; its post-pass only fills a plan if an external/non-boundary
interjection lacks one.
"""
...
class InterjectionsAndSpeechModule:
    ...
    def _mid_episode_interjections(
        ...
        what early runs surfaced as low-quality counterfactual interjections.
        ...
        co-timestamped plan state → action stream all line up.
        """
        ...
        if len(subtask_spans) < 2:
            ...
            return []
        # Deterministic per-episode RNG so reruns are stable across parallel workers.
        ...
        for t, prev_subtask, next_subtask in chosen:
            ...
            images = self.frame_provider.frames_at(record, window_ts, fail_on_error=True)
            image_blocks = to_image_blocks(images)
            if not image_blocks or len(images) != len(window_ts):
                raise RuntimeError(f"episode {record.episode_index}: interjection requires decoded frames")
