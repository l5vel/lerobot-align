...
per camera (upstream LeRobot ships such a recipe as
``recipes/pi05_hirobot.yaml`` in its own repository, not in this one).
...
Question types covered: bbox, keypoint, count, attribute, spatial. The assistant's ``content`` is a JSON string
...
class GeneralVqaModule:
        ...
        for idx in anchor_idx:
            ...
            for camera in cameras:
                ...
                if not _has_image_block(messages):
                    raise RuntimeError(f"episode {record.episode_index}: no VQA image for {camera} at {ts}")
    ...
    def _build_messages(
        ...
        images = self.frame_provider.frames_at(
            record, [frame_timestamp], camera_key=camera_key, fail_on_error=True
        )
