...
class EpisodeStaging:
    ...
    def write(self, module: ModuleName, rows: Iterable[dict[str, Any]]) -> Path:
        ...
        # half-written JSONL file that ``read()`` would then fail to parse.
        # A unique sibling also prevents concurrent runs from truncating each
        # other's temporary file before the atomic rename.
