#!/usr/bin/env bash
# Generate subtasks with a VLM, followed by plan/memory/task augmentations,
# for each dataset listed in a text file.
set -uo pipefail
umask 077

usage() {
    cat <<'EOF'
Usage: scripts/run_generate_batch.sh REPO_LIST_FILE
...
LOG_DIR="${LOG_DIR:-${XDG_STATE_HOME:-$HOME/.local/state}/lerobot-align/generate-batch}"
DST_OWNER="${DST_OWNER:-}"
...
SCRIPT_SHA="$(sha256sum "$SCRIPT_DIR/run_generate_batch.sh" | cut -d' ' -f1)"
RECIPE_FINGERPRINT="$(
    printf '%s\n' \
        'mode=generate' \
...
while IFS= read -r REPO || [[ -n "$REPO" ]]; do
    ...
    DST="$DST_OWNER/$KEY"
    # Keep the two batch modes' mutable work and completion state isolated.
    # They may intentionally target the same Hub repo, but must never make one
    # another look complete or delete one another's local worktree.
    WORK="$WORK_ROOT/$KEY.generate"
    LOG="$LOG_DIR/$KEY.log"
    DONE="$WORK_ROOT/$KEY.generate.done"
    ...
    safe_remove_tree "${WORK}_old" || exit 2
    safe_remove_tree "${WORK}_v30" || exit 2
    ...
    if [[ "$VERSION" == "v2.1" ]]; then
        if ! "$PY" -c 'import importlib.util,sys; sys.exit(importlib.util.find_spec("lerobot.scripts.convert_dataset_v21_to_v30") is None)'; then
            echo "  ERROR: LeRobot does not provide the v2.1-to-v3.0 converter." >&2
            echo "  Install a compatible LeRobot release in $VENV_DIR and retry." >&2
            FAILED+=("$KEY:converter-missing")
            continue
        fi

        echo "  converting v2.1 -> v3.0"
        if ! "$PY" -m lerobot.scripts.convert_dataset_v21_to_v30 \
            --repo-id="$REPO" --root="$WORK" --push-to-hub=false >>"$LOG" 2>&1
        then
            echo "  ERROR: conversion failed; see $LOG" >&2
            tail -20 "$LOG" >&2
            FAILED+=("$KEY:convert")
            continue
        fi
        if ! NEW_VERSION="$(read_version "$WORK/meta/info.json")" || [[ "$NEW_VERSION" != "v3.0" ]]; then
            echo "  ERROR: conversion did not produce a v3.0 dataset" >&2
            FAILED+=("$KEY:convert-version")
            continue
        fi
        safe_remove_tree "${WORK}_old" || exit 2
        safe_remove_tree "${WORK}_v30" || exit 2
    elif [[ ! "$VERSION" =~ ^v3\.[0-9]+([.][0-9]+)*$ ]]; then
        echo "  ERROR: unsupported dataset codebase_version: $VERSION" >&2
        FAILED+=("$KEY:unsupported-version")
        continue
    fi

    echo "  annotating with VLM subtask generation (log: $LOG)"
    START=$SECONDS
    "$ANNOTATE" \
        --root="$WORK" \
        --new_repo_id="$DST" \
        ...
        --allow_version_tag_move="$ALLOW_VERSION_TAG_MOVE" \
        --push_commit_message="Generate subtasks and plan/memory/task augmentation" \
        --job.target=local \
        ...
        --plan.frames_per_second=3 \
        --plan.max_frames_per_prompt=300 \
        --plan.plan_max_steps=12 \
        --plan.contact_sheet_frame_width=336 \
    ...
    if ! DONE_TMP="$(mktemp "$WORK_ROOT/.$KEY.generate.done.XXXXXX")"; then
    ...
    if ! printf 'repo=%s\nrevision=%s\ndst=%s\nelapsed=%s\nmode=generate\npackage=%s\nrecipe=%s\n%s\n' \
        ...
        "$RECIPE_FINGERPRINT" "$VLINE" >"$DONE_TMP" \
        || ! mv -f -- "$DONE_TMP" "$DONE"
    then
