...
class SyntheticFrameProvider:
    """Small real images for tests of module behavior independent of decoding."""
    ...
    def frames_at(self, record, timestamps, camera_key=None, *, fail_on_error=False):
        ...
        return [Image.new("RGB", (32, 24), (int(t * 10) % 256, 80, 120)) for t in timestamps]

    def video_for_episode(self, record, max_frames, camera_key=None):
        return self.frames_at(record, list(record.frame_timestamps[:max_frames]), camera_key)
