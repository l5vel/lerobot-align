...
# ``pyarrow`` and the package's ``lerobot.datasets`` dependency chain
...
from lerobot_align.writer import (  # noqa: E402
...
def test_writer_normalize_rejects_misrouted_persistent_style() -> None:
    ...
    from lerobot_align.writer import _normalize_persistent_row
...
def test_writer_normalize_rejects_misrouted_event_style() -> None:
    ...
    from lerobot_align.writer import _normalize_event_row
...
def test_say_tool_schema_constant_is_well_formed() -> None:
    ...
    """
    from lerobot_align.writer import (
        DEFAULT_TOOLS,
        SAY_TOOL_SCHEMA,
    )
...
def test_annotation_metadata_sync_drops_legacy_features(fixture_dataset_root: Path, tmp_path: Path) -> None:
    """A feature the writer dropped must not survive in ``meta/info.json``.

    The failure this guards against is silent rather than loud: metadata that
    still declares ``subtask_index`` after the writer removed the column makes
    ``LeRobotDataset`` return ``None`` for that key on every frame instead of
    raising, so downstream consumers read a fabricated value.
    """
    from lerobot_align.executor import Executor
    ...
    info["features"] = {
        ...
        "timestamp": {"dtype": "float32", "shape": (1,), "names": None},
        "task_index": {"dtype": "int64", "shape": (1,), "names": None},
        # declared by the source recording; the writer strips the column
        "subtask_index": {"dtype": "int64", "shape": (1,), "names": None},
        "observation.images.cam": {"dtype": "video", "shape": (3, 96, 96), "names": None},
    }
    info_path.write_text(json.dumps(info, indent=2))
    ...
    Executor._ensure_annotation_metadata_in_info(fixture_dataset_root, dropped_columns=writer.dropped_columns)

    synced = load_info(fixture_dataset_root)
    written = pq.read_table(next((fixture_dataset_root / "data").rglob("*.parquet")))

    assert "subtask_index" not in written.column_names
    assert "subtask_index" not in synced["features"], "info.json still advertises a column the writer removed"
    # video streams live in videos/, never as parquet columns -- must survive
    assert "observation.images.cam" in synced["features"]
    assert "task_index" in synced["features"]


def test_annotation_metadata_sync_keeps_subtask_index_when_writer_does(
    fixture_dataset_root: Path, tmp_path: Path
) -> None:
    """With the drop disabled, metadata and parquet must both keep the column."""
    from lerobot_align.executor import Executor
    from lerobot.datasets.io_utils import load_info
    ...
    records = list(iter_episodes(fixture_dataset_root))
    writer = LanguageColumnsWriter(drop_existing_subtask_index=False)
    writer.write_all(records, staging_dir, fixture_dataset_root)
    ...
    assert "subtask_index" in written.column_names
    assert "subtask_index" in synced["features"]
...
def test_writer_one_row_group_per_episode(tmp_path: Path) -> None:
    ...
    for ep in range(len(episode_lengths)):
        _stage_episode(
            ...
            interjections=[speech_atom(0.0, f"original speech {ep}")],
        )
...
def test_writer_partial_run_preserves_unselected_episode_annotations(tmp_path: Path) -> None:
    """Selecting one episode in a packed shard must not erase its neighbors."""
    root = _build_packed_dataset(tmp_path / "ds", [3, 4])
    shard = root / "data" / "chunk-000" / "file-000.parquet"
    staging_dir = tmp_path / "stage"
    for ep in (0, 1):
        _stage_episode(
    ...
    writer = LanguageColumnsWriter()
    writer.write_all(list(iter_episodes(root)), staging_dir, root)
    before = pq.read_table(shard)
    before_episode_1 = [
        (persistent, events)
        for ep, persistent, events in zip(
            before.column("episode_index").to_pylist(),
            before.column("language_persistent").to_pylist(),
            before.column("language_events").to_pylist(),
            strict=True,
        )
        if ep == 1
    ]
    ...
    writer.write_all(list(iter_episodes(root, only_episodes=(0,))), staging_dir, root)

    after = pq.read_table(shard)
    after_episode_1 = [
        (persistent, events)
        for ep, persistent, events in zip(
            after.column("episode_index").to_pylist(),
            after.column("language_persistent").to_pylist(),
            after.column("language_events").to_pylist(),
            strict=True,
        )
        if ep == 1
    ]
    assert after_episode_1 == before_episode_1
    episode_0 = next(
        rows
        for ep, rows in zip(
            after.column("episode_index").to_pylist(),
    ...
    assert [row["content"] for row in episode_0] == ["replacement episode 0"]


def test_writer_rejects_first_partial_run_inside_packed_legacy_shard(tmp_path: Path) -> None:
    """Selecting one episode must not implicitly migrate its packed neighbors."""
    root = _build_packed_dataset(tmp_path / "ds", [3, 4])
    shard = root / "data" / "chunk-000" / "file-000.parquet"
    staging_dir = tmp_path / "stage"
    _stage_episode(
    ...
    with pytest.raises(ValueError, match="without --only_episodes"):
        LanguageColumnsWriter().write_all(
            list(iter_episodes(root, only_episodes=(0,))),
            staging_dir,
            root,
        )
...
def test_writer_rejects_first_partial_run_that_would_split_dataset_schema(
    fixture_dataset_root: Path, tmp_path: Path
) -> None:
    """Untouched legacy shards require a full schema migration first."""
    staging_dir = tmp_path / "stage"
    _stage_episode(
        staging_dir,
        0,
    ...
    records = list(iter_episodes(fixture_dataset_root, only_episodes=(0,)))
    shards = sorted((fixture_dataset_root / "data").rglob("*.parquet"))
    before = {path: path.read_bytes() for path in shards}

    with pytest.raises(ValueError, match="without --only_episodes"):
        LanguageColumnsWriter().write_all(records, staging_dir, fixture_dataset_root)

    assert {path: path.read_bytes() for path in shards} == before
