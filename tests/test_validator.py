...
from lerobot_align.writer import speech_atom  # noqa: E402
...
def test_validator_catches_inconsistent_plan_memory(fixture_dataset_root: Path, tmp_path: Path) -> None:
    ...
    report = _validate(fixture_dataset_root, staging_dir)
    # missing co-timestamped plan state at 0.4s → error
    assert not report.ok
    assert any("co-timestamped plan state" in e for e in report.errors)
