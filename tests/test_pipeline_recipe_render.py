...
from tests._helpers import SyntheticFrameProvider  # noqa: E402
...
def _build_executor() -> Executor:
    vlm = make_canned_responder(
        {
            "Reconstruct the sequence of COMPLETED manipulation events": {
                "subtasks": [
            ...
            "alternative phrasings": {
                "rephrasings": [
                    ...
                    "fill the cup from the bottle",
                    "please pour the bottle's water into the cup",
                    "transfer water from the bottle to the cup",
                    "could you pour water into the cup",
                    "use the bottle to fill the cup",
                    "pour the water from this bottle into the cup",
                    "kindly fill the cup with water from the bottle",
                    "move the water from the bottle into the cup",
                    "empty the bottle's water into the cup",
                ]
            },
    ...
    return Executor(
        config=config,
        plan=PlanSubtasksMemoryModule(vlm=vlm, config=config.plan, frame_provider=SyntheticFrameProvider()),
        interjections=InterjectionsAndSpeechModule(vlm=vlm, config=config.interjections, seed=config.seed, frame_provider=SyntheticFrameProvider()),
