...
from lerobot_align.frames import (  # noqa: E402
    FrameProviderError,
    VideoFrameProvider,
...
def test_make_frame_provider_preserves_initialization_error_for_required_frames(
    tmp_path: Path, monkeypatch, caplog: pytest.LogCaptureFixture
) -> None:
    """A native caller can recover the error that the optional provider factory catches."""
    import lerobot_align.frames as frames_mod
    root_error = PermissionError("cannot read dataset metadata")

    def fail_provider(*_args, **_kwargs):
        raise root_error

    monkeypatch.setattr(frames_mod, "VideoFrameProvider", fail_provider)
    with caplog.at_level("WARNING"), pytest.raises(
        FrameProviderError, match="PermissionError.*cannot read dataset metadata"
    ) as info:
        make_frame_provider(tmp_path)
    assert "PermissionError" in caplog.text
    assert info.value.__cause__ is root_error
...
def test_decode_missing_per_episode_camera_metadata_returns_empty(
    tmp_path: Path, monkeypatch, caplog
) -> None:
    provider = _provider_for_video(tmp_path, tmp_path / "unused.mp4", monkeypatch)
    provider._meta.episodes[0] = {}

    with caplog.at_level("WARNING"):
        frames = provider._decode(0, [0.0], "observation.images.cam")

    assert frames == []
    assert "observation.images.cam" in caplog.text


def test_required_frames_surface_decode_root_error(tmp_path: Path, monkeypatch) -> None:
    """Strict/native reads raise with camera context instead of becoming an empty list."""
    from types import SimpleNamespace

    provider = _provider_for_video(tmp_path, tmp_path / "missing.mp4", monkeypatch)
    provider._meta.episodes[0] = {}
    record = SimpleNamespace(episode_index=0, frame_timestamps=[0.0])

    with pytest.raises(FrameProviderError, match=r"episode=0.*observation\.images\.cam") as info:
        provider.frames_at(record, [0.0], fail_on_error=True)

    assert isinstance(info.value.__cause__, KeyError)
    assert "episode metadata" in str(info.value)
...
def test_frames_at_decodes_nonzero_annotation_clock_from_zero_based_clip(
    sample_video: Path, tmp_path: Path, monkeypatch
) -> None:
    """Episode timestamps need not share the source clip's zero origin."""
    from types import SimpleNamespace

    provider = _provider_for_video(tmp_path, sample_video, monkeypatch)
    record = SimpleNamespace(
        episode_index=0,
        frame_timestamps=[10.0 + i / 10 for i in range(30)],
    )

    frames = provider.frames_at(
        record,
        [10.0, 11.2, 12.5],
...
@pytest.mark.parametrize(
    "decoded",
    [pytest.param([], id="decode-failure"), pytest.param(["only-first-miss"], id="partial-batch")],
)
def test_frames_at_returns_no_cache_survivors_when_miss_batch_is_incomplete(
    tmp_path: Path, monkeypatch, decoded: list[object]
) -> None:
    """A failed miss batch must not strip positions from warm-cache hits.

    Regression: with only the final timestamp cached, filtering ``None`` slots
    returned that final frame as the first item. Callers then paired it with
    the first requested timestamp and burned an incorrect timestamp badge.
    """
    from types import SimpleNamespace

    provider = _provider_for_video(tmp_path, tmp_path / "unused.mp4", monkeypatch)
    camera_key = "observation.images.cam"
    cached_late_frame = object()
    provider._cache[(0, camera_key, 2.0)] = cached_late_frame
    decode_calls = []
    ...
    def fake_decode(
        ...
        *,
        ...
        decode_calls.append((episode_index, timestamps, target))
        return decoded

    monkeypatch.setattr(provider, "_decode", fake_decode)
    record = SimpleNamespace(episode_index=0, frame_timestamps=[0.0, 1.0, 2.0])

    frames = provider.frames_at(record, [0.0, 1.0, 2.0], camera_key=camera_key)

    assert decode_calls == [(0, [0.0, 1.0], camera_key)]
    assert frames == []


def test_multiview_contact_sheet_stacks_synchronized_views_in_one_tile() -> None:
    red = Image.new("RGB", (100, 60), color=(255, 0, 0))
    blue = Image.new("RGB", (100, 60), color=(0, 0, 255))

    blocks = to_multiview_contact_sheet_blocks(
        [("camera-a", [red]), ("camera-b", [blue])],
        [1.25],
        columns=1,
        frames_per_sheet=1,
        frame_width=100,
    ...
    assert len(blocks) == 1
    sheet = blocks[0]["image"]
    assert sheet.size == (100, 120)
    top_pixel = sheet.getpixel((10, 40))
    bottom_pixel = sheet.getpixel((10, 100))
    assert top_pixel[0] > top_pixel[2]
    assert bottom_pixel[2] > bottom_pixel[0]


def test_encode_frames_to_clip_produces_decodable_timed_mp4(tmp_path: Path) -> None:
    """The native-video encoder must produce the timing the VLM consumes."""
    av = pytest.importorskip("av", reason="PyAV is required for native video encoding")
    try:
        av.codec.Codec("libx264", "w")
    except Exception as exc:
        pytest.skip(f"PyAV has no libx264 encoder: {exc}")

    frames = [
        Image.new("RGB", (64, 48), color=color)
        for color in ((255, 0, 0), (0, 255, 0), (0, 0, 255), (255, 255, 0))
    ]
    timestamps = [10.0, 10.5, 11.0, 11.5]
    clip_path = tmp_path / "native-generation.mp4"

    fps = encode_frames_to_clip(frames, timestamps, clip_path, frame_width=64)
    ...
    with av.open(str(clip_path)) as container:
        ...
        decoded_times = [float(frame.time) for frame in decoded]
        average_rate = float(stream.average_rate)

    assert len(decoded) == len(frames)
    assert decoded_times == pytest.approx([0.0, 0.5, 1.0, 1.5], abs=1e-3)
    assert average_rate == pytest.approx(2.0)
    assert all((frame.width, frame.height) == (64, 48) for frame in decoded)
...
def test_metadata_cannot_read_media_outside_dataset_root(tmp_path, monkeypatch, kind):
    from types import SimpleNamespace
    root = tmp_path / 'dataset'
    root.mkdir()
    private = tmp_path / 'private.mp4'
    private.write_bytes(b'private media that must never be decoded or sent')
    if kind == 'symlink':
        path = root / 'video.mp4'
        path.symlink_to(private)
    elif kind == 'traversal':
        path = Path('../private.mp4')
    else:
        path = private
    provider = _provider_for_video(root, path, monkeypatch)
    provider._meta.episodes[0]["videos/observation.images.cam/to_timestamp"] = 1.0
    monkeypatch.setattr('lerobot_align.frames.decode_video_frames',
                        lambda *a, **k: pytest.fail('external media was opened'))
    monkeypatch.setattr('lerobot_align.frames.reencode_video',
                        lambda *a, **k: pytest.fail('external media was reencoded'))
    with pytest.raises(FrameProviderError, match='escapes the dataset root'):
        provider._decode(0, [0.0], 'observation.images.cam', fail_on_error=True)
    with pytest.raises(FrameProviderError, match='escapes the dataset root'):
        provider.episode_clip_path(SimpleNamespace(episode_index=0), root / 'clips')
